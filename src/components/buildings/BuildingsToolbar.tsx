...
import type { BuildingSortField } from '../../types/building';
...
interface BuildingsToolbarProps {
    ...
    onSortChange: (field: BuildingSortField) => void;
}

const pageSizeOptions = [5, 10, 20, 30, 40, 50, 100];

const sortOptions: { field: BuildingSortField; label: string }[] = [
    { field: 'BuildingAddress', label: 'Edificio' },
    { field: 'BuildingSize', label: 'Superficie' },
    { field: 'BuildingPropertiesCount', label: 'Proprietà' },
    { field: 'BuildingComments', label: 'Descrizione' },
];

type OpenDropdown = null | 'pageSize' | 'sort';

export function BuildingsToolbar({
    pageSize,
    onPageSizeChange,
    searchQuery,
    onSearchChange,
                            ...
                            >
                                {sortOptions.map((opt) => (
                                    <button
                                        ...
                                        className="w-full text-left px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-50 cursor-pointer flex items-center gap-2 transition-colors"
                                    >
